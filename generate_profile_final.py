...
for s in netease_liked:
    ...
    if 'id=' in url:
        ...
        if sid not in song_ids:
            song_ids[sid] = s
unique_songs = list(song_ids.values())

# ========== 内容类型分析 ==========
# 豆瓣：从标题解析
douban_types = {'电影': 0, '电视剧': 0}
for d in douban:
    title = d.get('title', '')
    main_title = title.split('/')[0].strip()
    # 根据标题判断
    if any(kw in main_title for kw in ['电影', '动画', '动漫']):
        douban_types['电影'] += 1
    elif any(kw in main_title for kw in ['剧', '综艺']):
        douban_types['电视剧'] += 1
    else:
        douban_types['电影'] += 1  # 默认算电影
...
for s in unique_songs:
    ...
    if '陶喆' in artist or '方大同' in artist or '林俊杰' in artist or '王力宏' in artist:
        ...
        song_styles['流行/华语'] += 1
    elif 'Aimer' in artist or 'LiSA' in artist or 'YOASOBI' in artist:
        song_styles['日系动漫'] += 1
    else:
        song_styles['其他'] += 1
...
def get_recommendations():
    recs = {'movies': [], 'anime': [], 'songs': []}
    
    # 电影推荐：基于历史、推理、科幻偏好
    if anime_genres.get('历史', 0) > 5:
        recs['movies'].extend([
            {'title': '长安三万里', 'reason': '中国历史动画电影，与你喜欢的历史题材匹配'},
            {'title': '归来兮', 'reason': '历史剧情片'},
        ])
...
profile = {
    ...
    'stats': {
        ...
        'douban_total': len(douban),
        'douban_movies': douban_types['电影'],
        'douban_tv': douban_types['电视剧'],
    ...
    'top_songs': sorted(
        [json.loads(r[0]) for r in conn.execute('SELECT data_json FROM archive WHERE platform="netease" AND status="play_history"').fetchall()]
        if False else netease_history,
        key=lambda x: x.get('play_count', 0), reverse=True
    )[:20],
}

with open('/tmp/media_profile_final.json', 'w') as f:
    json.dump(profile, f, ensure_ascii=False, indent=2)

print("画像数据已生成:")
print(f"  豆瓣: {len(douban)}条 (电影{douban_types['电影']}, 电视剧{douban_types['电视剧']})")
...
print(f"  推荐: 电影{len(recommendations['movies'])}部, 番剧{len(recommendations['anime'])}部, 歌曲{len(recommendations['songs'])}首")
